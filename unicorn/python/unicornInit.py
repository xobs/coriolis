...
def unicornConfigure(**kw):
    ...
    if 'editor' in kw:
        ...
        print(ErrorMessage(
            3, 'unicornConfigure.py: Must be run from a CellView derived class.'))
        return
    ...
    if not cumulusDir:
        print(ErrorMessage(
            3, 'unicornConfigure.py: Cannot find <cumulus> in PYTHONPATH.'))
        return

    pluginsDir = os.path.join(cumulusDir, 'plugins')
    if not os.path.isdir(pluginsDir):
        print(ErrorMessage(
            3, 'unicornConfigure.py: Cannot find <cumulus/plugins> directory:', '<%s>' % pluginsDir))
        return
